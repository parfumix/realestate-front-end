<template>
      ...
      <div v-for="(message, index) in messages" :key="index" class="flex">
        <div v-if="message.sender === 'user'" class="ml-auto relative">
          <p class="bg-blue-500 text-white py-2 px-4 rounded-lg shadow">
            ...
            <span v-if="isQueryLoading && (index+1 == messages.length)" class="absolute top-0 right-0 flex h-3 w-3"><span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-sky-400 opacity-75"></span><span class="relative inline-flex rounded-full h-3 w-3 bg-sky-500"></span></span>
          </p>
        </div>
  ...
  <div class="w-full rounded-b-lg flex justify-between border-t border-gray-300 ">
    ...
    <button :disabled="isQueryLoading" @click="() => handleSendMessage(message)" class="ml-2 bg-blue-500 text-white py-2 px-4 rounded-lg hover:bg-blue-600 flex items-center">
      <svg v-if="!isQueryLoading" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6"><path stroke-linecap="round" stroke-linejoin="round" d="m15 11.25-3-3m0 0-3 3m3-3v7.5M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>
...
const { isQueryLoading } = storeToRefs(chatStore)
