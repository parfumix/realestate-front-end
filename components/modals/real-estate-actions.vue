<template>
    <div class="flex justify-between items-center py-2 mr-4">
        ...
        <div class="flex items-center justify-end space-x-4">
            <div v-for="(action, index) in actions" :key="`${index}-${item.is_favorited}`" class="cursor-pointer">
                <span @click="action.handler" v-html="typeof action.icon == 'string' ? action.icon : action.icon()"></span>
            </div>
        </div>
    </div>
</template>
...
const actions = ref([{
    handler: async() => {
        ...
        chatStore.handleUpdateItem(item.value.id, {
            is_favorited: !item.value.is_favorited
        })

        await toggleFavorite(user.value?.id, item.value.id)
    },
    icon: () => {
        return !item.value.is_favorited 
