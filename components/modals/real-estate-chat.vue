<template>
    <div class="h-full">
        <Chat-Property @submit="handleSendMessage" :isLoading="isQueryLoadingProperty" :messages="defaultThreadMessages">
            <template #header>
            ...
            <div>
                <div v-if="!defaultThreadMessages.length" :class="[tabs.length > 1 ? 'border-b border-gray-200' : '', 'mx-auto flex justify-center']">
                    <nav class="-mb-px flex space-x-[12px]" v-if="tabs.length > 1">
                        <a v-for="(tab, index) in tabs.filter(el => el.visible)" :key="tab.title" class="cursor-pointer py-4 px-1 flex items-center" @click="selectTab(tab.slug)" :class="{ 'border-b border-gray-500 text-blue-500': activeTab === index }">
                ...
                <div class="mx-auto flex flex-wrap justify-center my-2">
                    <span v-for="(prompt, index) in prompts" @click="() => handleSelectPrompt(prompt)" :key="index" class="cursor-pointer text-center inline-flex items-center mt-2 rounded-md bg-gray-100 px-4 py-2 text-xs font-medium text-gray-600 mr-2">
                        {{ prompt }}
                    </span>
                </div>
...
const { loading: isQueryLoadingProperty } = storeToRefs(propertyQuestionStore)
