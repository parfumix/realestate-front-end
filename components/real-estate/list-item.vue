<template>
    <div @mouseover="() => handleHoverItem(item.id)" @mouseleave="() => handleHoverItem(null)" class="shadow rounded-md bg-white">
        <div @click="handleOpen" class="group aspect-h-6 aspect-w-10 block w-full overflow-hidden rounded-lg bg-gray-100 focus-within:ring-2 focus-within:ring-blue-500 focus-within:ring-offset-2 focus-within:ring-offset-gray-100">
            <img :src="`${item.images?.[0]}`" :alt="item.description" class="pointer-events-none w-full h-40 object-cover group-hover:opacity-75" />
        </div>
        <div class="px-3 py-2 flex flex-col justify-start">
            <div class="flex justify-between">
                <div class="flex-1">
                    <p @click="handleOpen" class="text-sm text-left font-medium text-gray-900 leading-5 pb-2 hover:underline cursor-pointer" :title="item?.title">{{ (item?.title || '').substring(36, item?.title) + '…' }}</p>
                </div>
...
const handleOpen = () => {
    window.open(`/property/${props.item.slug}`, '_blank')
}
